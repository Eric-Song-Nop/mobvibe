...
- 拆分 hooks：`useSessionEventSources`（SSE）。
...
- 新增 hooks：`useSessionEventSources` 负责 SSE 订阅。
...
## 本次调整（实现前计划 - 2026-01-18 代码预览树状大纲）

- 目标：文件预览的 `code` 模式增加 Tree-sitter 大纲面板与语义复制能力。
- 交互：桌面端右侧固定大纲面板，移动端顶部切换“代码/大纲”。
- 能力：点击大纲项滚动定位代码行，长按大纲项复制对应语义区域。
- 结构：多层级树形展示，支持折叠/展开。
- 技术：前端引入 `web-tree-sitter` + TS/JS 语法 wasm，按需解析生成大纲。
- 架构：`CodePreview` 内新增大纲解析与树形渲染逻辑，代码区维持 Prism 高亮。
- 兼容：仅支持 JS/TS/TSX/JSX，复制文本不包含行号。

## 本次调整（实现后记录 - 2026-01-18 代码预览树状大纲）

- `CodePreview` 引入 Tree-sitter 解析，按语言加载 JS/TS/TSX wasm 并生成大纲树。
- 桌面端右侧固定大纲面板，移动端通过顶部按钮切换“代码/大纲”。
- 大纲支持多层级折叠，点击节点滚动到对应行。
...
## 本次调整（实现前计划 - 2026-01-20 代码预览大纲测试修复）

- 对齐 `CodePreview` 大纲类型按钮的可访问名称与测试断言。
- 更新点击复制的测试选择器，避免被新的辅助文案影响。
- 实现后补充测试修复记录与关键行为说明。

## 本次调整（实现后记录 - 2026-01-20 代码预览大纲测试修复）
