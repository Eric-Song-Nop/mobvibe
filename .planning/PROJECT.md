...
- [ ] 用户可以创建一个 team run，选择同一台机器、workspace、目标任务和一个 leader agent
- [ ] Leader agent 可以在用户确认后通过 team MCP tools 创建成员、拆分任务、分配任务和汇总结果
- [ ] 每个 team member 仍然是普通 ACP session，但只在 team session 的 `session/new` 中声明 team MCP server，以便使用 `mobvibe_team_*` 工具通信
- [ ] 普通非 team session 不声明 team MCP server、不修改全局 agent MCP 配置，因此用户在其他地方使用 agent 的行为不受影响
- [ ] Agents 可以通过 CLI 持久化 mailbox 互相发送消息，通过 task board 创建、领取、更新和完成任务
- [ ] 用户可以在 WebUI 中看到 team run 总览，包括 leader/member、agent、状态、task board、mailbox 活动、错误和需要处理的权限请求
...
- [ ] Gateway 和 CLI 可以用共享类型传递 team run 元数据、mailbox/task 事件和 team MCP readiness，而不引入 app-to-app 直接依赖
...
---
*Last updated: 2026-05-13 after Phase 1 决策上下文锁定*
