...
describe("agent team routes", () => {
	...
	it("lists and gets teams through TeamRouter", async () => {
		const list = await requestJson(
			baseUrl,
			"/acp/agent-teams?machineId=machine-1",
			{
				headers: { authorization: "Bearer user-1" },
			},
		);
		const get = await requestJson(
			baseUrl,
			"/acp/agent-teams/team-1?machineId=machine-1",
			{
				headers: { authorization: "Bearer user-1" },
			},
		);
	...
	it("maps ownership and timeout failures without leaking another machine", async () => {
		teamRouter.listAgentTeams.mockRejectedValueOnce(
		...
		const ownership = await requestJson(
			baseUrl,
			"/acp/agent-teams?machineId=other",
			{
				headers: { authorization: "Bearer user-1" },
			},
		);
